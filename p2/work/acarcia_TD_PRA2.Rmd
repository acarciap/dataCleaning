...
Se propone desarrollar un modelo a partir de la caracterización recogida en este set, que permita calificar vehículos utilizando criterios similares a los implícitos en la muestra recogida en los datos utilizados como insumo, completando en lo posible la brecha identificada en ellos.
...
El set de datos se asume autocontenido, por lo que no es necesario enriquecerlo o ni procesamientos adicionales en este sentido.
...
La existencia de observaciones con modelos y versiones previos del mismo fabricante (con valores distintos de cero para el atributo), aunado a que se tienen observaciones con el atributo inicializado a 0.00, que exhiben valor para el atributo _**CA**_, inducen a pensar de que este tipo de casos se constituyen una contradicción. 
...
Traslación de los estimados obtenidos para los atributos _**NOx**_ y _**GEI**_, inicilizados con centinelas de valor cero.
...
Esto se corrobora con los gráficos "quantile vs quantile" (qq) que se han incorporado para cada variable. En ellos se observa que en el mejor de los casos, la adherencia a la diagonal se afecta por distorsiones en los extremos o en el centro. 
...
Esta segmentación es la primera causa de diferencias tan pronunciadas en los casos identificados en atributos como la _**potencia**_ y la _**capacidad**_ del motor (_**tamaño**_), que a su vez repercute sobre el rendimiento del vehículo en relación al consumo de combustible.

Así mismo, una breve revisión en relación a la emisión de _**CO2**_ y _**NOx**_, nos permite apreciar que los motores **Diesel** son los que reportan en promedio altos valores para estos atributos, seguidos luego por modelos gran _**potencia**_ a _**Gasolina**_. 
...
## 4.3 Exportación de Datos Procesados
...
## 4.4 Distribución de la Variable de Respuesta
...
El resultado revela un fuerte desequilibrio en relación a la forma en que se distribuyen las proporciones de la variable calificación de _**Contaminación de Aire**_ sobre la muestra, concentrando la mayor cantidad de marcas, modelos y versiones en las categorías más contaminantes.

## 4.5 Distribución de Variables Categóricas
...
La gráfica por Tipo de Transmisión, presenta un comportamiento similar a la anterior, con una concentración mayor de modelos construidos con cajas de cambio Automáticas; pero con una distribución dispersa sobre las categorías de _**Contaminación del Aire**_.
...
En el caso del _**Tipo de Transmisión**_, aunque persiste el desbalance en la distribución con respecto a las categorías de _**Contaminación del Aire**_, el efecto se percibe menos intenso.
...
Esta variable se aprecia mejor distribuida en el rango de categorías de la variable _**Contaminación del Aire**_; sin embargo, debido a su carácter estructural, el desbalance en la distribución persiste como puede apreciarse en las tabulaciones de frecuencia y proporciones:
...
## 4.6 Distribución de Variables Continuas
...
Finalmente, para el _**rendimient en Ciudad**_ tenemos una situación muy paracida, con bastante más difererncia en las formas de las distribuciones parciales, con una aparente concentración alrededor de los 12 Km por litro.
...
## 4.7 Selección de Atributos de la Muestra

La siguiente tabulación de niveles de _**Emisión de Gases de Efecto Invernadero**_, por calificación de _**Contaminación de Aire**_ nos permite apreciar mejor la muy característica ausencia de valores (inicializados a cero) para un segmento importante de la muestra.
...
Producto de la reducción de filas, una revisión de las proporciones a través de una tabulación simple de variables categóricas del set, revela que la variable _**comb**_ deja de tener una distribución de niveles que resulte relevante o razonable para su consideración
...
## 4.8 Normalización de Variables Numéricas

Como parte de las tareas de Limpieza y adecuación de datos, incluimos el escalamiento de las variables numéricas, de manera que en caso de aplicar métodos que impliquen cálculo de distancias, la diferencia de escalas no distorsione los resultados.
...
Habiendo excluído el _**Tipo de Combustible**_, las variables categóricas restantes _**Tipo de Transmisión**_, _**Número de Cilindros**_ y _**Emisión de Gases de Efecto Invernadero**_, pueden ser utilizadas para la conformación de grupos que puedan ayudar a entender el comportamiento del modelo.
...
_Análisis a aplicar_.
  ...
  * Verificación de la condición de homogeneidad de varianza de los atributos continuos, para los grupos que resulten de la segmentación por variable categórica.
...
La condición de distribución normal de las variables tiene incidencia en las pruebas de inferencia y contraste de significancia de los parámetros de los modelos. Sin embargo, el volumen de la muestra puede hacer menos sensibles los test a la ausencia de normalidad.
...
Considerando que los grupos a inspeccionar, determinados por las variables categóricas _**Tipo de Transmisión**_, _**Número de Cilindros**_ y _**Emisión de Gases de Efecto Invernadero**_, se aplicará la prueba de Levene de Homogeneidad de Varianza, sobre las variables continuas _**pot**_ y _**NOx**_, que son las que hasta ahora parecen prometedoras en relación a su capacidad predictiva en relación a la calificación de _**Contaminación de Aire**_ 

```{r Evaluacion de Levene para pot por atributo Categórico  }
### Evaluación de Levene para la Potencia por atributo Categórico 
###
leveneTest(y=cga2018.dat$pot, group=cga2018.dat$trans, center="median")
leveneTest(y=cga2018.dat$pot, group=cga2018.dat$cil, center="median")
leveneTest(y=cga2018.dat$pot, group=cga2018.dat$GEI, center="median")
```

Los resultados obtenidos de la aplicación de la prueba sobre la variable _**pot**_, para los diferentes establecidos, resultan en valores para el p-valor por debajo del nivel de significación de 0.05, que nos lleva a rechazar la hipótesis nula del test (de igualdad de varianza) de la variable continua para cada uno de los grupos.

```{r Evaluacion de Levene para NOx por atributo Categórico  }
### Evaluación de Levene para NOx por atributo Categórico 
###
leveneTest(y=cga2018.dat$NOx, group=cga2018.dat$trans, center="median")
leveneTest(y=cga2018.dat$NOx, group=cga2018.dat$cil, center="median")
leveneTest(y=cga2018.dat$NOx, group=cga2018.dat$GEI, center="median")
```

Para el caso de la variable _**NOx**_, observamos el mismo resultado.

Finalmente, para las pruebas sobre el _**rendimiento en Ciudad**_ se tiene:
```{r Evaluacion de Levene para r.Ciudad por atributo Categórico  }
### Evaluación de Levene para r.Ciudad por atributo Categórico 
###
leveneTest(y=cga2018.dat$r.Ciudad, group=cga2018.dat$trans, center="median")
leveneTest(y=cga2018.dat$r.Ciudad, group=cga2018.dat$cil, center="median")
leveneTest(y=cga2018.dat$r.Ciudad, group=cga2018.dat$GEI, center="median")
```

En conclusión, para todos los casos estudiados, la variable continua tiene varianza distinta en al menos alguno de los niveles, en el dominio de cada variable categórica utilizada para la definición de los grupos de interés. 

Por lo tanto, en todos los casos probados, se rechaza la hipótesis nula de la prueba, en beneficio de la alternativa que establece la hetereoscedasticidad de las variables para cada uno de los grupos.

La aplicación de la prueba de fligner (no paramétrica, y no sensible a la normalidad de los atributos) arroja resultados similares en cuanto a la heteroscedasticidad de la muestra:
```{r Evaluacion de fillgner para r.Ciudad por atributo Categórico, echo=FALSE}
### Evaluación de Levene para r.Ciudad por atributo Categórico 
###

f1 <- fligner.test(pot ~ trans, data= cga2018.dat)
f2 <- fligner.test(pot ~ cil, data= cga2018.dat)
f3 <- fligner.test(pot ~ GEI, data= cga2018.dat)

field.pot <- c(f1$p.value,f2$p.value,f3$p.value)

f1 <- fligner.test(tam ~ trans, data= cga2018.dat)
f2 <- fligner.test(tam ~ cil, data= cga2018.dat)
f3 <- fligner.test(tam ~ GEI, data= cga2018.dat)

field.tam <- c(f1$p.value,f2$p.value,f3$p.value)

f1 <- fligner.test(r.Ciudad ~ trans, data= cga2018.dat)
f2 <- fligner.test(r.Ciudad ~ cil, data= cga2018.dat)
f3 <- fligner.test(r.Ciudad ~ GEI, data= cga2018.dat)

field.cdad <- c(f1$p.value,f2$p.value,f3$p.value)

f1 <- fligner.test(CO2 ~ trans, data= cga2018.dat)
f2 <- fligner.test(CO2 ~ cil, data= cga2018.dat)
f3 <- fligner.test(CO2 ~ GEI, data= cga2018.dat)

field.co2 <- c(f1$p.value,f2$p.value,f3$p.value)

f1 <- fligner.test(NOx ~ trans, data= cga2018.dat)
f2 <- fligner.test(NOx ~ cil, data= cga2018.dat)
f3 <- fligner.test(NOx ~ GEI, data= cga2018.dat)

field.nox <- c(f1$p.value,f2$p.value,f3$p.value)

rslt <- rbind(field.pot,field.tam,field.cdad,field.co2,field.nox )
colnames(rslt) <- c("transmision","cilindros","GEI")
rslt
...
Producto de la ausencia de normalidad en las variables de la muestra, y la ausencia homoscedasticidad en los grupos propuestos, no puede aplicarse el análisis de varianzas a través de la anova, para validar la homogeneidad de las medias a través de los grupos de cada factor.

Nos restringiremos a revisar las relaciones entre columnas, y de las columnas versus la variable de respuesta, a través del estudio de la correlación que pueda existir entre los componentes; y revisar la media de las variables continuas más importantes, con respecto a la variable de respuesta.
...
Sin embargo, la presencia de "tie values" impiden una correcta determinación de rho, por lo que se ensaya el método de Kendall como alternativa, explorando la determinación de tau-b como indicador de correlación:
...
El atributo _**NOx**_ exhibe un valor de 0.7 para el coeficiente de correlación, que confirma su relación con la variable de respuesta. Sin embargo, para el resto de las variables, el coeficiente obtenido es bastante próximo a cero, lo que indica ausencia de correlación.

### 5.3.2 Correlacionadas entre variables continuas
...
La gráfica revela que hay correlaciones entre pares de atributos, que nos permitirán simplificar el tamaño de la muestra: Tanto la _**potencia**_  como la _**capacidad**_ del motor, están positivamente correlacionadas con la emisión de _**CO2**_.
...
De la revisión de la relación de correlación entre las variables numéricas, resulta la simplificación del conjunto inicial, para continuar con los atributos _**CO2**_, _**NOx**_ y _**Rendimiento en Ciudad**_.

Media del atributo _**CO2**_ por nivel del factor correspondiente a la salida se lista de seguido:
...
for (i in 1:length(levels(cga2018.dat$CA))) {
  ...
  print( paste( "Contaminación Aire: cat. ", 
...
De los resultados obtenidos en los distintos listados previos, los atributos _**CO2**_ y _**NOx**_ parecen exhibir buenas diferencias en sus medias, por nivel del factor de salida. No obstante, la variable _**rendimiento en Ciudad**_ presenta valores más homogéneos. 

Para este último caso es pruedente verificar si estadísticamente la variación en la media observada por categoría es estadísticamente significativa, y decidir si el campo poseen suficiente información para incorporarlo en el modelo de clasificación. 


_**Varia la media del rendimiento en Ciudad para el grupo de autos con CA por debajo de la categoría 5, con respecto al resto de la clasificación ?**_
...
##   y el resto por debajo de 5 
##
md.cdad.bajos <- subset(cga2018.dat, CA == 1 | 
                                     CA == 2 | 
                                     CA == 3 | 
                                     CA == 4 |
                                     CA == 5 ,
...
md.cdad.altos <- subset(cga2018.dat, CA == 6 | 
                                     CA == 7 |
                                     CA == 8 | 
                                     CA == 9 | 
                                     CA == 10 ,
...
El resultado puede estar influido por la distribución de observaciones por categoría de _**CA**_ que hemos ya identificado en secciones previas.

_**Varían entre si las medias de a pares de categorías, para las categorías superiores?**_

Para este tipo de preguntas pudiera aplicarse el análisis de varianza, si las condiciones estuvieran dadas; pero para nuestra muestra (sin normalidad y heteroscedasticidad), tendremos que aplicar el contraste de medias para muestras distintas a pares.
...
##
md.cdad.bajos <- subset(cga2018.dat, CA == 6, select=c(r.Ciudad)) 
md.cdad.altos <- subset(cga2018.dat, CA == 7, select=c(r.Ciudad)) 

cat.6.vs.7 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA == 7, select=c(r.Ciudad)) 
md.cdad.altos <- subset(cga2018.dat, CA == 8, select=c(r.Ciudad)) 

cat.7.vs.8 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA == 8, select=c(r.Ciudad)) 
md.cdad.altos <- subset(cga2018.dat, CA == 9, select=c(r.Ciudad)) 

cat.8.vs.9 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA == 6, select=c(r.Ciudad)) 
md.cdad.altos <- subset(cga2018.dat, CA ==9, select=c(r.Ciudad)) 

cat.6.vs.9 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

rslt <- cbind(cat.6.vs.7$p.value, cat.7.vs.8$p.value, cat.8.vs.9$p.value, cat.6.vs.9$p.value)
colnames(rslt) <- c("6.vs.7", "7.vs.8", "8.vs.9", "6.vs.9")
rslt
```

El resultado para cada una de las comparaciones da por encima del nivel de significación, por lo que no puede rechazarse la igualdad de media entre estas categorías.

_**Varia la media del CO2 para el grupo de autos con CA por debajo de la categoría 5, con respecto al resto (para el subgrupo de autos de cuatro cilindros) ?**_

La pregunta va en línea con la intención original de utilizar las variables categóricas seleccionadas de la muestra para la evaluación de grupos de interés.
...
#### Comparación de la media del atributo CO2 para los grupos superiores de CA
##   y el resto por debajo de 5, en coches de 4 cilindros 
##
md.cdad.bajos <- subset(cga2018.dat, (CA==1|CA==2|CA==3|CA==4|CA==5) & (cil==4),select=c(CO2)) 
md.cdad.altos <- subset(cga2018.dat, (CA==6|CA==7|CA==8|CA==9|CA==10) & (cil==4),select=c(CO2)) 

t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)
```

Al igual que en el ejemplo previo, el test obliga a rechazar la hipótesis de igualdad de medias para ambos grupos (categorías bajas de _**CA**_, versus la agrupación de las altas). 
...
##
md.cdad.bajos <- subset(cga2018.dat, CA==6 & cil==4, select=c(CO2)) 
md.cdad.altos <- subset(cga2018.dat, CA==7 & cil==4, select=c(CO2))

cat.6.vs.7 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA==7 & cil==4, select=c(CO2))
md.cdad.altos <- subset(cga2018.dat, CA==8 & cil==4, select=c(CO2))

cat.7.vs.8 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA==8 & cil==4, select=c(CO2))
md.cdad.altos <- subset(cga2018.dat, CA==9 & cil==4, select=c(CO2))
...
cat.7.vs.9 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)


rslt <- cbind(cat.6.vs.7$p.value, 
              cat.7.vs.8$p.value, 
              cat.8.vs.9$p.value, 
              cat.6.vs.9$p.value, 
              cat.7.vs.9$p.value)

colnames(rslt) <- c("6.vs.7", "7.vs.8", "8.vs.9", "6.vs.9", "7.vs.9")
rslt
```

A excepción de la categoría 7, que durante su comparación con las 8 y 9, obtiene p-valores que permiten afirmar que tienen medias distintas, para el resto de las comparaciones no hay evidencia de las medias sean significativamente distintas para las categorías 6, 8 y 9. 

_**Dentro del grupo de categorías altas para CA serán distintas las medias del atributo NOx entre las categorías, para coches con transmisión automática ?**_

El poco poder predictivo que intuímo para los atributos _**rendimiento Ciudad**_ y _**CO2**_, producto de las pruebas anteriores, nos conduce a cuestionarnos el potencial de la variable _**NOx**_. Por ella razón planteamos esta última pregunta, y aplicando el mismo enfoque previo:

```{r Revisión de variacion de media categoria superior CA- atr NOx }
#### Comparación a pares de la media del atributo NOx para los grupos superiores
##   en autos de transmisión automática
##
md.cdad.bajos <- subset(cga2018.dat, CA==6 & trans=="Auto", select=c(NOx)) 
md.cdad.altos <- subset(cga2018.dat, CA==7 & trans=="Auto", select=c(NOx)) 

cat.6.vs.7 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA==7 & trans=="Auto", select=c(NOx)) 
md.cdad.altos <- subset(cga2018.dat, CA==8 & trans=="Auto", select=c(NOx)) 

cat.7.vs.8 <- t.test( md.cdad.bajos, md.cdad.altos, var.equal=FALSE, paired=FALSE)

md.cdad.bajos <- subset(cga2018.dat, CA==8 & trans=="Auto", select=c(NOx)) 
md.cdad.altos <- subset(cga2018.dat, CA==9 & trans=="Auto", select=c(NOx)) 
...
## 6. Modelo Tentativo
En el siguiente segmento se procede a extraer una parte del conjunto de datos para utilizarlo como set de pruebas, para validar la precisión del modelo generado.
...
# Homologación de los niveles de test
# por la baja representatividad de la categoria en los datos
# debemos asegurarnos que el niveleste en el caonjunto de test
# a modo que las predicciones puedan ser comparable con el modelo

levels(m.test$CA) <- c(levels(m.test$CA), "10")   

m.test.rsl <- m.test$CA

## se eliminan los atributos redundantes o que han demostrado baja contribución 
## por su escaso poder predictivo, además de la variable objetivo
##

m.training_1 <- m.training[ , c( -3, -4, -5, -6, -7, -8, -12)]
m.test_1 <- m.test[ , c( -3, -4, -5, -6, -7, -8, -12)]
...
Un método no supervisado como el kmeans pudiera ser de utilidad, pero en su versión para datos mixtos (numéricos y categóricos), cuya implementación está disponible a través de la función **kproto**. 

En vista de que existen 10 categorías registradas para el atributo _**CA**_, impondremos k = 10 para la inicialización del hiperparámetro del modelo.

```{r Modelo k-proto para nuestra muestra }
#### kmeans para data mixta: numérica y categórica
##
mv <- kproto(m.training_1, 10)

#clprofiles(mv, m.training_1)

m.predicted.clusters <- predict(mv, m.test_1)
...
La precisión obtenida es demasiado baja como para promover el modelo, lo que nos reafirma el caracter poco predictivo de los datos y sesgo que estructuralmente se ha venido apreciando en los datos disponibles a lo largo de las pruebas estadísticas previas.

Alternativamente, consideraremos un modelo de árbol que ayude a ilustrar mejor la realidad de las caracteríticas de nuestro set de datos:

```{r Modelo arbol CART para nuestra muestra }
#### Modelo de árbol CART 
##
mv <- rpart(CA~NOx+CO2+trans+cil+GEI, data=m.training, method="class")

printcp(mv) # resultados del modelo
plotcp(mv)  # visualización de resultados cruzados
```

El atributo _**NOx**_ es considerado como suficiente para la confección del árbol, cuestión que intuitivamente se anticipaba a partir del estudio de las medias, realizado en secciones previas.

El nivel de complejidad que minimiza el error es que corresponde al CP 7, tal como se ve en la tabulación de resultados y la gráfica correspondiente.

```{r Modelo arbol CART para nuestra muestra - grafica}
#### Modelo de árbol CART 
##
plot(mv, uniform=TRUE, main="Clasificación de CA")
text(mv, use.n=TRUE, all=TRUE, cex=.6)
```

Utilizando el conjunto reservado para el test, y estimar la precisión del modelo:
...
La tasa de precisión que alcanza este nuevo modelo basado en un árbol CART, establece lo adecuado que resulta para el problema que nos hemos planteado.
...
Específicamente, cualquier intento de tratamiento de los casos para los valores reportados en relación a las emisiones (presuntos outliers), solo puede ser procesadas bajo conocimiento funcional del dominio del problema, de nodo que las acciones que se puedan adelantar en este sentido, sean coherentes y no desvirtúen la calidad de los datos. No obstante, una breve investigación sobre el tema, nos hacen pensar que los niveles reportados, son correctos y son similares a mediciones practicadas por instituciones como la ADAC.
...
Lamentablemente el set de datos presenta un déficit estructural con respecto a la casuística que debería considerarse dentro del dominio del problema, ya que no se dispone de suficientes valores para para uno de los estratos de una de las variables, además de que para algunas categorías de la variable de respuesta se aprecia un desbalanceo en lo referente a la disponibilidad de casos etiquetados para algunas de las categorías:

    Particualarmente, el modelo para la calificación del nivel de la Contaminación del Aire que 
    hemos obtenido, para la predicción del nivel de contaminación producido por vehículos a
    Gasolina, no puede ser aplicado a observaciones correspondientes a modelos que consumen
